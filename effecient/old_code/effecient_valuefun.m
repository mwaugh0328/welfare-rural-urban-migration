...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    v_prime_rural_not(zzz,1) = sum(pitest.*(sigma_nu_not.*log(1./max(pitest, 10^-25))  + [v_stay_rural_not, v_move_seasn_not, v_move_rural_not]),2);
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    
    pitest = [move.rural_exp(zzz,1),  diff(move.rural_exp(zzz,:),1,2)];
    
    v_prime_rural_exp(zzz,1) = sum(pitest.*(sigma_nu_not.*log(1./max(pitest, 10^-25))  + [v_stay_rural_exp, v_move_seasn_exp, v_move_rural_exp]),2);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    pitest = [move.urban_new(zzz,1), diff(move.urban_new(zzz,:),1,2)];
    
    v_prime_urban_new(zzz,1) = sum(pitest.*(sigma_nu_not.*log(1./max(pitest, 10^-25))  + [v_stay_urban_new, v_move_urban_new]),2);
