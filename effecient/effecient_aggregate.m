...
mpl.raw.rural.monga = params.alpha.*(labor.supply.monga).^(params.alpha-1);
...
mpl.raw.rural.notmonga = params.alpha.*(labor.supply.notmonga).^(params.alpha-1);
...
iwage.rural = (mpl.rural.monga.*sum((1./(params.rural_tfp.*tfp.monga)).*rural_monga_work_rural(:,production)./number_workers) + ...
    mpl.rural.notmonga.*sum((1./(params.rural_tfp.*tfp.notmonga)).*rural_not_monga_work_rural(:,production)./number_workers) + ...
