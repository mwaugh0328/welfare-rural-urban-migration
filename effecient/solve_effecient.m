...
%best = load('anlyz_guess.mat','x0');
best = load('move_psearch_best_rents_85.mat','x1');
...
% tic
% 
% opts = optimset('Display','iter','UseParallel',true,'MaxFunEvals',5000,'TolFun',10^-3,'TolX',10^-3);
%  
% x1 = fminsearchcon(@(xxx) compute_effecient(xxx, new_cal, tfp, 0), x1,LB,UB,A,b,[],opts);
% 
% [social_welfare] = compute_effecient(x1, new_cal, tfp, 1);
% 
% save move_fminsearch_anlytic x1
% 
% toc
...
% 
% x1 = fminsearchcon(@(xxx) compute_effecient(xxx, exp(new_val), tfp, 0), best.x0,LB,UB,A,b,[],opts);
% 
