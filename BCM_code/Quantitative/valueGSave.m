...
%G=1/(1-parm.beta)*((parm.x - parm.sub+parm.m).^(1-parm.sigma)/(1-parm.sigma));
CG  = parm.yhat;
ppG = spline(parm.x,G);
...
while crit > parm.epsilon
    for i=1:length(parm.x)
        [CG_new(i),val] = fminbnd(@(c) objGSave(c,parm.x(i),parm, G), parm.sub,parm.x(i) + parm.m);
        G_new(i) = -val;
    end
