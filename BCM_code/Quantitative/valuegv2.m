...
while crit >epsilon
    ...
    for i=1:xgrid
        [c_G_new(i) val]=fminbnd(@(c) objGv2(c ,x(i), parm, G), parm.sub, x(i)+parm.m);% upper bound is adjusted because they will migrate before they choose consumption levels.
    ...
    disp(crit) ;
    disp(crit_percent)
    disp(max(G)) % display iteration statissavetics in percentage + or -
      look = G;
      look2 = G_new;
