function [V,c,ppc,ppV, V_no] = valuev(parm, x, epsilon, ymean, yl, xgrid, ppG, ppB);
...
while crit > epsilon
    for i = 1: xgrid
      [c_new(i), val] = fminbnd(@(c) objV(c,x(i),parm,ppV), parm.sub, x(i));
      V_new(i) = max((parm.piG*ppval(ppG,x(i)) + (1-parm.piG)*(ppval(ppB,x(i)-parm.F))-parm.UC),-val); %i am not 100% on what this should be - should it inlcude the cost and benefit of migration or not.  Incentive enters here conceptually.
