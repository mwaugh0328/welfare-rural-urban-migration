...
els_moments = zeros(n_params,n_params + 2);
jacobian_moments = zeros(n_params,n_params + 2);
...
for xxx = 1:n_params
        ...
        % variance.
        moments_for(10) = moments_for(10)^2 + me_consumption_var; % use measurement error and change from sd to var as in varcovar matrix
        moments_bak(10) = moments_bak(10)^2 + me_consumption_var; % use measurement error and change from sd to var as in varcovar matrix
...
test = jacobian_moments(:,order_moments);
...
V=[experiment_var_cov  zeros_7x3
   zeros_3x7         aggregate_var_cov];
...
disp([params me_consumption_var me_wage_var; SD])
